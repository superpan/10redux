...
export default function App() {
  ...
  const [playing, setPlaying] = useState(null) // {clip_id, t_start, name}
  ...
  return (
    <div className="app">
      ...
      {hits.length === 0 ? (
        ...
        <div className="grid">
          {hits.map(h => <Card key={h.clip_id} hit={h} onPlay={setPlaying} />)}
        </div>
      )}
  ...
  return (
    <div className="card">
      <img
        ...
        alt=""
        onClick={() => onPlay({ clip_id: p.clip_id, t_start: p.t_start, name: p.video_name })}
      />
      <div className="body">
        <div className="meta">
        ...
        <div className="name" title={p.video_path}>{p.video_name}</div>
        <div className="caption">{p.caption}</div>
        <div className="actions">
          <button onClick={() => onPlay({ clip_id: p.clip_id, t_start: p.t_start, name: p.video_name })}>
            ▶ play
          </button>
...
function Player({ clip, onClose }) {
  ...
  useEffect(() => {
    const v = ref.current
    if (!v) return
    const onMeta = () => { v.currentTime = clip.t_start; v.play().catch(() => {}) }
